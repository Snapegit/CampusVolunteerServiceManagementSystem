	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.huodongbianhao" placeholder="活动编号"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('zhiyuanzhehuodong','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('zhiyuanzhehuodong','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('zhiyuanzhehuodong','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('zhiyuanzhehuodong','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				ref="table"
				v-if="btnAuth('zhiyuanzhehuodong','查看')"
				:data="list"
				...
				<el-table-column label="活动图片" width="120" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="审核" v-if="btnAuth('zhiyuanzhehuodong','审核')" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('zhiyuanzhehuodong','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('zhiyuanzhehuodong','报名参加')" type="success" @click="huodongcanjiaCrossAddOrUpdateHandler(scope.row,'cross','否','','')">报名参加</el-button>
						<el-button v-if="btnAuth('zhiyuanzhehuodong','查看评论')" type="warning" @click="commentClick(scope.row.id)">查看评论</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<huodongcanjiaFormModel ref="huodongcanjiaFormModelRef" @formModelChange="formModelChange"></huodongcanjiaFormModel>
	</div>
</template>
<script setup>
	...
	//基础信息
	const tableName = 'zhiyuanzhehuodong'
	const formName = '志愿者活动'
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.huodongbianhao&&searchQuery.value.huodongbianhao!=''){
			params['huodongbianhao'] = '%' + searchQuery.value.huodongbianhao + '%'
		}
	...
	const approvalClick = (row) => {
		let params = {
			...
			huodongtiaojian: row.huodongtiaojian,
	...
	const commentClick=(id)=>{
		context?.$router.push('/discusszhiyuanzhehuodong?refid=' + id)
	}
	import huodongcanjiaFormModel from '@/views/huodongcanjia/formModel'
	const huodongcanjiaFormModelRef = ref(null)
    const huodongcanjiaCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			huodongcanjiaFormModelRef.value.init(row.id,'cross','报名参加',row,'zhiyuanzhehuodong',statusColumnName,tips,statusColumnValue)
		})
    }
	//查询审核状态列表
