...
<template>
	<div>
			...
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						...
						<el-button v-if="btnAuth('xuesheng','加积分')" type="success" @click="jiajifenCrossAddOrUpdateHandler(scope.row,'cross','','','')">加积分</el-button>
						<el-button v-if="btnAuth('xuesheng','减积分')" type="success" @click="jianjifenCrossAddOrUpdateHandler(scope.row,'cross','','','')">减积分</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<jiajifenFormModel ref="jiajifenFormModelRef" @formModelChange="formModelChange"></jiajifenFormModel>
		<jianjifenFormModel ref="jianjifenFormModelRef" @formModelChange="formModelChange"></jianjifenFormModel>
	</div>
</template>
<script setup>
	...
	import jiajifenFormModel from '@/views/jiajifen/formModel'
	const jiajifenFormModelRef = ref(null)
    const jiajifenCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			jiajifenFormModelRef.value.init(row.id,'cross','加积分',row,'xuesheng',statusColumnName,tips,statusColumnValue)
		})
    }
	import jianjifenFormModel from '@/views/jianjifen/formModel'
	const jianjifenFormModelRef = ref(null)
    const jianjifenCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
			var obj = row
			for (var o in obj){
				if(o==statusColumnName && obj[o]==statusColumnValue){
		...
		nextTick(()=>{
			jianjifenFormModelRef.value.init(row.id,'cross','减积分',row,'xuesheng',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
