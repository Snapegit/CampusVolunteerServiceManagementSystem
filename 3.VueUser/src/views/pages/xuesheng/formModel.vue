	...
	<div class="app-contain" :style='{"padding":"0px 0","margin":"20px auto 40px","alignItems":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","display":"flex","width":"86%","position":"relative","justifyContent":"space-between"}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth(tableName,'加积分')" @click="jiajifenonAcross('加积分','jiajifen','','','')" type="warning">加积分</el-button>
					<el-button class="cross_btn" v-if="btnFrontAuth(tableName,'减积分')" @click="jianjifenonAcross('减积分','jianjifen','','','')" type="warning">减积分</el-button>
...
<script setup>
	...
	const jiajifenonAcross = (btnType,table,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
	...
	const jianjifenonAcross = (btnType,table,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('请登录后再操作！','error')
			return false
		}
